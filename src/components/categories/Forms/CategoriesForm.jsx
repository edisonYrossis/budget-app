...
import {faLaptop, faPencil, faFlag, faBoltLightning, faUserTie, faBookBookmark, faGlobe, faBaseballBatBall, faHeartPulse, faSyringe, faCalculator, faMoneyCheckDollar, faMoneyBill1Wave, faEarthAfrica, faChartSimple ,faHeart, faStar, faCoffee, faTrainTram, faTrainSubway, faBus, faCar, faGraduationCap, faHouse, faUtensils, faTv, faUserGroup, faDumbbell, faBagShopping, faMobileScreenButton, faGift, faMusic } from '@fortawesome/free-solid-svg-icons';
...
function CategoriesForm({setCategoryFormView, categoryFormView}) {

  const {dispatch, CATEGORIES_ACTIONS, labelsCategoryArray, setLabelsCategoryArray, predefinedColors, currentColor, setCurrentColor} = useCategoriesContextAll()
    ...
    const handleSubmit = (e)=> {
        ...
        dispatch({type: CATEGORIES_ACTIONS.ADD_CATEGORY, payload: {
        ...
        Swal.fire({
          ...
          setCategoryFormView(false)
          setLabelsCategoryArray([])
      }
  return (
    <div className={ categoryFormView ? 'fixed top-0 left-0 w-full h-screen backdrop-blur-sm bg-black bg-opacity-20 flex justify-center items-center z-50 ' : 'hidden'}>
    ...
    <main className='relative bg-gray-100 rounded-xl shadow-2xl h-4/5 w-fit flex flex-col justify-center items-center py-8 px-2 overflow-y-scroll max-w-screen-md dark:text-black'>
        ...
        <h1 className='font-semibold text-xl mb-2'>Category Form</h1>
       ...
       <button className={`bg-green-100 rounded-lg py-2 px-4 font-semibold text-2xl ${transactionState == 'income' ? 'bg-green-300' : 'bg-green-100 opacity-50'}`} onClick={() => setTransactionState('income')}>Income</button>
        ...
        <span className='w-full flex flex-col items-center gap-0.5'> 
              <label className='font-semibold ' >Category name</label> 
                <input type="text" className='w-full h-12 border border-black px-6 dark:text-white' value={categoryTitle} onChange={(e) => setCategoryTitle(e.target.value)} placeholder='Health, Home...' />
              </span>
...
<div className="mb-5 mt-1 flex flex-col gap-0.5 justify-center w-fit px-4 py-1 rounded-lg" style={{backgroundColor: currentColor}}>
                    <label className='text-black font-semibold'>Selected Icon</label>
                        <FontAwesomeIcon icon={categoryIcon} size="3x" />
                    </div>
